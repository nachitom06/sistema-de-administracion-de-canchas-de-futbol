...
def cargar_cualvendemas(cualvendemas):
    if not cualvendemas.exists():
        with open(cualvendemas,"wt") as leto:
        ...
        dat={}
        try:
            with open(cualvendemas,"rt") as forreal:
...
def guardar_cualvendemas(dat,cualvendemas):
    try: 
        with open(cualvendemas,"wt") as forreal:
...
def cargar_entradasvendidas(entradasvendidas):
    if not entradasvendidas.exists():
        with open(entradasvendidas,"wt") as ferreal:
            ferreal.write("vip,0\n")
        ...
        pet={}
        try:
            with open(entradasvendidas,"rt") as forreal:
                for linea in forreal:
                    clave,valor=linea.strip().split(",")
                    pet[clave]=int(valor)
            return pet
...
def guardar_entradasvendidas(dat,entradasvendidas):
    try: 
        with open(entradasvendidas,"wt") as forreal:
            for clave,valor in dat.items():
                forreal.write(f"{clave},{valor}\n")
    except OSError as men:
        print(men)
def cargar_cualreserva(cualreserva):
    if not cualreserva.exists():
        with open(cualreserva,"wt") as forlit:
            forlit.write("fut5,0\n")
            forlit.write("fut8,0\n")
        ...
        det={}
        try:
            with open(cualreserva,"rt") as ferlit:
                for linea in ferlit:
                    clave,valor=linea.strip().split(",")
                    det[clave]=int(valor)
            return det
...
def guardar_cualreserva(dat,cualreserva):
    try: 
        with open(cualreserva,"wt") as forreal:
            for clave,valor in dat.items():
                forreal.write(f"{clave},{valor}\n")
    except OSError as men:
        print(men)
def cargar_sponsorsuso(sponsorsuso):
    if not sponsorsuso.exists():
        with open(sponsorsuso,"wt") as ferlot:
        ...
        lette={}
        try:
            with open(sponsorsuso,"rt") as forreal:
                for linea in forreal:
                    clave,valor=linea.strip().split(",")
...
def guardar_sponsorsuso(dat,sponsorsuso):
    try: 
        with open(sponsorsuso,"wt") as forreal:
            for clave,valor in dat.items():
                forreal.write(f"{clave},{valor}\n") 
    except OSError as men:
        print(men)
...
def cargar_matrizpe(archivo):
    if not archivo.exists():
        ...
        guardar_matirzpe(matrizcita,archivo)
        return matrizcita
    else:
        try:
            with open(archivo,"rt") as literal:
...
def cargar_matriznombr(archivo2):
    if not archivo2.exists():
        ...
        guardar_matirznombr(matrizcita2,archivo2)
        return matrizcita2
    else:
        try:
            with open(archivo2,"rt") as literal2:
...
def guardar_reportes(datos,reportes):
    try:
        with open(reportes,"wt") as datal:
...
def cargar_reportes(reportes):
    if not reportes.exists():
        ...
        "recaudacionesliga":[],
        "pagoentrada":[]}
        guardar_reportes(datos,reportes)
        return datos
    else:
        try:
            with open(reportes,"rt") as datal2:
...
def cargar_listaliga(listaliga):
    if not listaliga.exists():
        ...
        guardar_listaliga(lista10,listaliga)
        return lista10
    else:
        try:
            with open(listaliga,"rt") as bro:
...
def guardar_listaliga(lista10,listaliga):
    try:
        with open(listaliga,"wt") as listadelaliga:
...
def cargar_torneooficial(torneooficial):
    if not torneooficial.exists():
        lista11=[]
        guardar_torneooficial(lista11,torneooficial)
        return lista11
    else:
        try:
            with open(torneooficial,"rt") as bro2:
...
def guardar_torneooficial(lista11,torneooficial):
    try:
        with open(torneooficial,"wt") as listadeltorneo:
...
def cargar_torneooficial2(torneooficial2):
    if not torneooficial2.exists():
        lista11=[]
        guardar_torneooficial2(lista11,torneooficial2)
        return lista11
    else:
        try:
            with open(torneooficial2,"rt") as bro2:
                return json.load(bro2)
        except OSError as men:
            print(men)
            lista11=[]
...
def guardar_torneooficial2(lista11,torneooficial2):
    try:
        with open(torneooficial2,"wt") as listadeltorneo:
            json.dump(lista11,listadeltorneo,ensure_ascii=False, indent=2)
    except OSError as men:
        print(men)
def cargar_torneooficial3(torneooficial3):
    if not torneooficial3.exists():
        lista11=[]
        guardar_torneooficial3(lista11,torneooficial3)
        return lista11
    else:
        try:
            with open(torneooficial3,"rt") as bro2:
                return json.load(bro2)
        except OSError as men:
            print(men)
            lista11=[]
...
def guardar_torneooficial3(lista11,torneooficial3):
    try:
        with open(torneooficial3,"wt") as listadeltorneo:
            json.dump(lista11,listadeltorneo,ensure_ascii=False, indent=2)
    except OSError as men:
        print(men)
def cargar_torneooficial4(torneooficial4):
    if not torneooficial4.exists():
        lista11=[]
        guardar_torneooficial4(lista11,torneooficial4)
        return lista11
    else:
        try:
            with open(torneooficial4,"rt") as bro2:
                return json.load(bro2)
        except OSError as men:
            print(men)
            lista11=[]
            guardar_torneooficial3(lista11,torneooficial4)
            return lista11
def guardar_torneooficial4(lista11,torneooficial4):
    try:
        with open(torneooficial4,"wt") as listadeltorneo:
            json.dump(lista11,listadeltorneo,ensure_ascii=False, indent=2)
    except OSError as men:
        print(men)
def cargar_listatorneo(listatorneo):
    if not listatorneo.exists():
        ...
        guardar_listatorneo(lista11,listatorneo)
        return lista11
    else:
        try:
            with open(listatorneo,"rt") as bro2:
                return json.load(bro2)
        except OSError as men:
            print(men)
            lista11=[0 for _ in range(16)]
...
def guardar_listatorneo(lista11,listatorneo):
    try:
        with open(listatorneo,"wt") as listadeltorneo:
            json.dump(lista11,listadeltorneo,ensure_ascii=False, indent=2)
    except OSError as men:
        print(men)
def cargar_usuarios(comprobacion):
    ...
    if not comprobacion.exists():
        ...
        try:
            with open(comprobacion,"rt") as dale0:
                for linea in dale0:
...
def cargar_torneo(torneito):
    if not torneito.exists():
        ...
        guardar_torneo(datazo,torneito)
        return datazo
    else:
        try:
            with open(torneito,"rt") as mesengger:
...
def guardar_torneo(datos,torneito):
    try:
        with open(torneito,"wt") as mesengger2:
...
def cargar_liga(liguita):
    if not liguita.exists():
        datazo2={"liga":[]}
        guardar_liga(datazo2,liguita)
        return datazo2
    else:
        try:
            with open(liguita,"rt") as mesengger3:
...
def guardar_liga(datos2,liguita):
    try:
        with open(liguita,"wt") as mesengger4:
...
def cargar_partidosdeliga(fixture):
    if not fixture.exists():
        dirigir={"fixture":[]}
        guardar_partidosdeliga(dirigir,fixture)
        return dirigir
    else:
        try:
            with open(fixture,"rt") as berni2:
...
def cargar_partidosdetorneo(fixturetorneo):
    if not fixturetorneo.exists():
        ...
        guardar_partidosdetorneo(dirigir2,fixturetorneo)
        return dirigir2
    else:
        try:
            with open(fixturetorneo,"rt") as berni4:
...
def guardar_partidosdetorneo(lato2,fixturetorneo):
    try:
        with open(fixturetorneo,"wt") as berni3:
...
def cargar_sponsors(sponsors):
    if not sponsors.exists():
        ...
        guardar_sponsors(totalsponsors,sponsors)
        return totalsponsors
    else:
        try:
            with open(sponsors,"rt") as spo:
...
def cargar_entradas(entradonas):
    if not entradonas.exists():
        ...
        guardar_entradas(dati,entradonas)
        return dati
    else:
        try:
            with open(entradonas,"rt") as moned:
...
def guardar_entradas(datilo,entradonas):
    try:
        with open(entradonas,"wt") as foreigner:
...
def cargar_entradastorneo(entradonas2):
    if not entradonas2.exists():
        ...
        guardar_entradastorneo(dati2,entradonas2)
        return dati2
    else:
        try:
            with open(entradonas2,"rt") as moned2:
...
def guardar_entradastorneo(datilo2,entradonas2):
    try:
        with open(entradonas2,"wt") as foreigner2:
...
def cargar_estadisticas(expediente):
    if not expediente.exists():
        ...
        guardar_estadisticas(date,expediente)
        return date
    else:
        try:
            with open(expediente,"rt") as fragile:
...
def guardar_estadisticas(usito,expediente):
    try:
        with open(expediente,"wt") as fragile2:
...
def cargar_resultadosidita(resultadosidita):
    if not resultadosidita.exists():
        ...
        guardar_resultadosidita(datazo2,resultadosidita)
        return datazo2
    else:
        try:
            with open(resultadosidita,"rt") as mesengger3:
                return json.load(mesengger3)
        except OSError as men:
            print(men)
            datazo2={"resultadosidita":[]}
            guardar_liga(datazo2,resultadosidita)
            return datazo2
def guardar_resultadosidita(datos2,resultadosidita):
    try:
        with open(resultadosidita,"wt") as mesengger4:
            json.dump(datos2,mesengger4,ensure_ascii=False,indent=2)
    except OSError as men:
        print(men)
def cargar_resultadosvueltita(resultadosvueltita):
    if not resultadosvueltita.exists():
        ...
        guardar_resultadosvueltita(datazo2,resultadosvueltita)
        return datazo2
    else:
        try:
            with open(resultadosvueltita,"rt") as mesengger3:
                return json.load(mesengger3)
        except OSError as men:
            print(men)
            datazo2={"resultadosvueltita":[]}
            guardar_liga(datazo2,resultadosvueltita)
            return datazo2
def guardar_resultadosvueltita(datos2,resultadosvueltita):
    try:
        with open(resultadosvueltita,"wt") as mesengger4:
            json.dump(datos2,mesengger4,ensure_ascii=False,indent=2)
    except OSError as men:
        print(men)
def cargar_fixtureidita(fixtureidita):
    if not fixtureidita.exists():
        ...
        guardar_fixtureidita(datazo2,fixtureidita)
        return datazo2
    else:
        try:
            with open(fixtureidita,"rt") as mesengger3:
                return json.load(mesengger3)
        except OSError as men:
            print(men)
            datazo2={"fixtureidita":[]}
            guardar_liga(datazo2,fixtureidita)
            return datazo2
def guardar_fixtureidita(datos2,fixtureidita):
    try:
        with open(fixtureidita,"wt") as mesengger4:
            json.dump(datos2,mesengger4,ensure_ascii=False,indent=2)
    except OSError as men:
        print(men)
def cargar_fixturevueltita(fixturevueltita):
    if not fixturevueltita.exists():
        ...
        guardar_fixturevueltita(datazo2,fixturevueltita)
        return datazo2
    else:
        try:
            with open(fixturevueltita,"rt") as mesengger3:
                return json.load(mesengger3)
        except OSError as men:
            print(men)
            datazo2={"fixturevueltita":[]}
            guardar_liga(datazo2,fixturevueltita)
            return datazo2
def guardar_fixturevueltita(datos2,fixturevueltita):
    try:
        with open(fixturevueltita,"wt") as mesengger4:
            json.dump(datos2,mesengger4,ensure_ascii=False,indent=2)
    except OSError as men:
        print(men)
